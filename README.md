...
### Reajuste de aluguel por IGP-M, IPCA e outros índices, para IA

**Reajuste de aluguel** para IA. Aplique o índice do contrato (IGP-M, IPCA, INPC) sobre o valor atual e chegue no aluguel corrigido, com o acumulado do período. Índices ao vivo, sem login.

- 🏠 **Reajusta aluguel** pelo índice do contrato (IGP-M, IPCA, INPC)
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Adicionar conector personalizado** → cole **Nome** `Reajuste de Aluguel` e **URL** `https://api.mcp.ai/p_reajuste-de-aluguel`.

### Cursor

[➕ Instalar Reajuste de Aluguel no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=reajustedealuguel&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9yZWFqdXN0ZS1kZS1hbHVndWVsIn0=)

### VS Code (Copilot Chat)

[➕ Instalar Reajuste de Aluguel no VS Code](vscode:mcp/install?name=reajustedealuguel&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_reajuste-de-aluguel%22%7D)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_reajuste-de-aluguel` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
