...
### Rent adjustment by IGP-M, IPCA and other indices, for AI

**Rent adjustment** for AI. Apply the contract index (IGP-M, IPCA, INPC) to the current rent and get the adjusted value, with the period's accrual. Live indices, no login.

- 🏠 **Adjusts rent** by the contract index (IGP-M, IPCA, INPC)
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Reajuste de Aluguel` and **URL** `https://api.mcp.ai/p_reajuste-de-aluguel`.

### Cursor

[➕ Install Reajuste de Aluguel in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=reajustedealuguel&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9yZWFqdXN0ZS1kZS1hbHVndWVsIn0=)

### VS Code (Copilot Chat)

[➕ Install Reajuste de Aluguel in VS Code](vscode:mcp/install?name=reajustedealuguel&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_reajuste-de-aluguel%22%7D)
...
```
Adjust a R$2,000 rent by the accumulated IGP-M of the last 12 months
What is the new rent corrected by IPCA from January to December?
```
...
- 📧 [reajustedealuguel@mcp.ai](mailto:reajustedealuguel@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/reajuste-de-aluguel-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_reajuste-de-aluguel` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
